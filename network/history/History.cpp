...
namespace Network {
	...
	Historys::Historys(const Historys& other) :
	...
	unsigned Historys::GetReceivedMessages() const
	{
		return receivedMessages;
	}

	uint64_t Historys::GetReceivedBytes() const
	{
		return receivedBytes;
	}

	unsigned Historys::GetSentMessages() const
	{
		return sentMessages;
	}

	uint64_t Historys::GetSentBytes() const
	{
		return sentBytes;
	}

	void Historys::RegisterSentMessage(int32_t num_bytes)
	{
		++sentMessages;
		sentBytes.fetch_add(num_bytes);
	}

	void Historys::RegisterReceivedMessage(int32_t messageSize)
	{
		++receivedMessages;
	...
	std::ostream& operator<<(std::ostream& os, const Historys& stat)
	{
