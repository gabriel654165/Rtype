...
static void compute_join_button(Registry &r, unsigned long distance, Resource::NtwClient &client)
{
    ...
    for (auto &it : tagList) {
        if (distance == it) {
            ...
            protocol.pack("dd", Network::Protocol::CodeTcp::ReqDisconnectFromLobby, client.getClient().GetTcpClient()->getInLobby().second);
...
{
    auto mouse = *r.get_tagged_entities("mouse_lobby_ready").begin();
