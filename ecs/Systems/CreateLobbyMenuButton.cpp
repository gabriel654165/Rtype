...
void create_button_anim_pressed(std::optional<Animations> &iteAnim)
{
    iteAnim->currentAnimation->second.currentFrame = 2;
}

void create_button_anim_fly_over(std::optional<Animations> &iteAnim)
{
...
void compute_create_button(Registry &r, unsigned long distance, Resource::NtwClient &client)
{
    auto tagList = r.get_tagged_entities("LinkToCreateNewLobby");
    for (auto &it : tagList) {
        if (distance == it) {
...
void set_create_button_state(Registry &r, std::optional<Controllable> &iteCtrl, std::optional<Animations> &iteAnim, unsigned long distance, Resource::NtwClient &client)
{
    if (iteCtrl->isMousePressed) {
        if (iteAnim.has_value()) {
            create_button_anim_pressed(iteAnim);
        } else {
   //         button_color_change_pressed();
        }
    } else {
        if (iteAnim.has_value()) {
            create_button_anim_fly_over(iteAnim);
        } else {
  //          button_color_change_fly_over();
        }
    }
    if (iteCtrl->isClicked) {
        compute_create_button(r, distance, client);
...
{
    auto mouse = *r.get_tagged_entities("mouse_create_menu").begin();
        ...
        if (cldr[id]->type == Collider::RECTANGLE) {
            ...
            if (controller->mousePosition.x >= pos[id]->x + colliderInfo.xoff
                ...
                if (controller->mousePosition.y >= pos[id]->y + colliderInfo.yoff
                    ...
                    set_create_button_state(r, controller, anim[id], id, client);
                } else {
