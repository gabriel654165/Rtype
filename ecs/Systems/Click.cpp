//
// Created by thomas on 15/11/2021.
//
...
void change_scene_by_tag(Registry &r, unsigned long distance, Resource::NtwClient &client)
{
    ...
    tagList.clear();
    tagList = r.get_tagged_entities("LinkToCreateJoinLobby");
    for (auto &it : tagList) {
        if (distance == it) {
            Network::Protocol::Manager protocol;
            protocol.pack("dd", Network::Protocol::CodeTcp::ReqCreateLobby, 5);
...
//            r.handle_scenes();
    ...
    tagList.clear();
    tagList = r.get_tagged_entities("LinkToReturnMainMenu");
    for (auto &it : tagList) {
        if (distance == it) {
            r.change_scene("menu");
            r.handle_scenes();
...
void set_button_state(Registry &r, std::optional<Controllable> &iteCtrl, std::optional<Animations> &iteAnim, unsigned long distance, Resource::NtwClient &client)
{
    ...
    if (iteCtrl->isClicked) {
        change_scene_by_tag(r, distance, client);
        iteCtrl->isClicked = false;
    }
}

void click_system([[maybe_unused]]Registry &r, Resource::NtwClient &client, sparseArray<Position> &pos, sparseArray<Collider> &cldr, sparseArray<Controllable> &ctrl, sparseArray<Animations> &anim)
{
    auto mouse = *r.get_tagged_entities("mouse").begin();
    auto &controller = ctrl[mouse];
    for (auto &id : r.getScene().entities) {
        if (!pos[id].has_value() || !cldr[id].has_value())
            continue;
        if (cldr[id]->type == Collider::RECTANGLE) {
            ...
            if (controller->mousePosition.x >= pos[id]->x + colliderInfo.xoff
                ...
                if (controller->mousePosition.y >= pos[id]->y + colliderInfo.yoff
                    ...
                    set_button_state(r, controller, anim[id], id, client);
                } else {
